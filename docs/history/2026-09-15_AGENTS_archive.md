...
> 이 파일은 2026-09-15 경량화 이전 지침의 보관본입니다. 아래 과거 규칙과 상태는 현재 지침이 아닙니다. 현재 규칙은 루트 AGENTS.md를 따르고, 필요한 주제만 검색하며 전체를 기본 로드하지 않습니다.
...
- 슈퍼 관리자의 사용자 진도·DID 리워드·대구체인 사용 로그 공통 사용자 조회는 Hibernate Criteria Query의 `NULLS LAST`를 사용하지 않습니다. 운영 Hibernate는 이 정렬을 지원하지 않아 세 API가 500으로 실패할 수 있으므로 전체 사용자를 조회한 뒤 `realOrganization` null-last, 사용자명 순으로 애플리케이션에서 정렬합니다.
