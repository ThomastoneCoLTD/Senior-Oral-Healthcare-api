...
- DID 생성 경로 기본값은 `/did/create`이며 회원가입 DID 생성 요청은 `label`에 사용자 로그인 아이디를 넣어 호출합니다. 로그인 credential은 DID 서버 `/did/issue-vc`에서 VC-JWT로 발급하고 로그인 시 `/did/verify-vc`로 검증합니다.
