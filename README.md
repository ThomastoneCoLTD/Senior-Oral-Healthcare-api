...
When SOH creates a raw DaeguChain reward wallet, it uses the returned wallet private key only in memory to approve the configured token owner for each `DAEGU_CHAIN_REWARD_TOKEN_CONTRACTS` contract, then discards the key. The approval payload's `holder_pkey` is masked in API audit logs. A one-time super-admin reset is restricted to the explicitly approved test wallet `0x64770d4a82ec450AA58875f86a3Ad977141C57A1fca`; it requires the exact current address and an existing failed reclaim record, and removes only the SOH reward wallet/transaction rows so the next DaDaegu login can provision a newly approved wallet.
